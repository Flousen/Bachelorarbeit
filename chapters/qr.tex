...
Der Vektor $v$ wird mit 
\begin{align*}
	v = \dfrac{x - \alpha e_1}{x_1 - \alpha}
\end{align*}
normiert, so dass $v_1 = 1$ gilt, um ihn später auf der frei werdenden Diagonalen von $A$ speichern zu können.
...
Die Matrix $A \in \mathbb{R}^{m \times n}$ wird nun geblockt, mit einer geeigneten Blockgröße $bs$ betrachtet.
\begin{align}
...
Aus den Householder-Vektoren wird einer obere Dreiecksmatrix $T \in \mathbb{R}^{bs \times bs}$ für die gilt $H_{0}\cdot ... \cdot H_{bs} = H = I - V*T*V^T$ berechnet.\\
